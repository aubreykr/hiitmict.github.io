...
# calculate % change VO2 max
percent_change_vo2 <- (vo2[vo2$Timepoint == "Endpoint",]$VO2_Peak_mLkg - vo2[vo2$Timepoint == "Baseline",]$VO2_Peak_mLkg) / vo2[vo2$Timepoint == "Baseline",]$VO2_Peak_mLkg * 100
record_id<-unique(vo2$record_id)
vo2_change<-data.frame(cbind(record_id, percent_change_vo2))
vo2<-merge(vo2, vo2_change)


# plot % change in VO2 max
vo2 %>% 
  filter(Timepoint=="Baseline") %>% 
  ggplot(aes(x=group, y=percent_change_vo2)) + geom_boxplot() + geom_hline(yintercept=0, linetype="dotted") + geom_point() + 
   stat_summary(
    fun = median,
    geom = "text",
    vjust = -0.6,
...
vo2 %>% 
  filter(Timepoint == "Baseline" & group!="Con" & record_id!="4") %>% 
  ggplot(aes(x=quality_workouts_in_REDCap, y=percent_change_vo2, color=group)) + geom_point() + 
...
vo2 %>% 
  filter(Timepoint == "Baseline" & group!="Con") %>% 
  ggplot(aes(x=total_workouts_in_REDCap, y=percent_change_vo2, color=group)) + geom_point() + 
...
#model<-lm(percent_change_vo2 ~ sex + agegroup + weight_kg + quality_workouts_in_REDCap + group, data = test)
#summary(model)
