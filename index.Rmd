...
num<-length(demog$record_id[demog$enrollment_status== "On-going Randomized" & demog$current_week !="Not Training Yet"])
```

Number of participants currently training: `r num`
...
theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) + xlab(paste0("Plot Last Updated:", sep=" ", Sys.Date())) + ggtitle("Who is currently training?") +   labs(fill = "Record ID") + theme(plot.title = element_text(hjust = 0.5))+ scale_y_continuous(breaks = seq(0, 30, 2)) + geom_text(aes(label = record_id, y = ..count..), 
            stat = "count", 
...
demog %>% 
  filter(enrollment_status == "On-going Randomized" & current_week !="Not Training Yet") %>%  
  dplyr::select(current_week, record_id, Group, sex) %>% 
