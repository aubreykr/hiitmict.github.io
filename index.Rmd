...
theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) + xlab(paste0("Plot Last Updated:", sep=" ", Sys.Date())) + ggtitle(paste0("Snapshot of Participant Progress:", sep=" ", Sys.Date())) +   labs(fill = "Randomization Group") + theme(plot.title = element_text(hjust = 0.5))+ scale_y_continuous(breaks = seq(0, 30, 2)) 
...
theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) + xlab(paste0("Plot Last Updated:", sep=" ", Sys.Date())) + ggtitle(paste0("Snapshot of Participant Progress:", sep=" ", Sys.Date())) +   labs(fill = "Participant ID") + theme(plot.title = element_text(hjust = 0.5)) + scale_y_continuous(breaks = seq(0, 30, 2)) 
...
```{r complete, message=FALSE, warning=FALSE, echo=FALSE}

completed<-cohort %>% 
  filter(enrollment_status == "Completed")

completed %>% 
  ggplot(aes(x=Group, fill=Group))+geom_bar(stat="count")+
  theme(plot.title = element_text(hjust = 0.5))+xlab("Group")+ylab("Number")+scale_fill_discrete(name = "Group")+facet_wrap(~strata)+
  geom_text(aes(label=..count..),stat='count',vjust=1.5) + 
  ggtitle(paste("Completed Study: n=", sep="", length(unique(completed$record_id)))) + guides(alpha = "none")
...
demog %>%
  filter(enrollment_status != "Screen failed" & enrollment_status != "Withdrawn" & enrollment_status != "Lost to Follow-Up") %>% 
  ggplot(aes(x = start_month, fill=factor(record_id), alpha=0.5)) + scale_x_date(date_breaks = "1 month", date_labels = "%b %Y") +
  ...
  scale_y_continuous(breaks = seq(0, 10, 1), limits = c(0, 10)) + labs(fill = "Participant ID")

# Projected end months
demog %>%
  filter(enrollment_status != "Screen failed" & enrollment_status != "Withdrawn" & enrollment_status != "Lost to Follow-Up") %>% 
  ggplot(aes(x = end_month, fill=factor(record_id), alpha=0.5)) +scale_x_date(date_breaks = "1 month", date_labels = "%b %Y") +
